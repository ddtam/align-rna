...
/*
 * Run STAR alignment on paired end reads. Uses samtools to convert SAM to BAM
 *      and sort alignments.
 *
 * INPUT:
 *      Tuple w/ sample ID, R1 reads, R2 reads,
 *      Path to STAR parsed splicesites,
 *      Root filenames of HISAT2 index files,
 *      Path to directory containing HISAT2 index files.
 *
 * OUTPUT:
...
process alignFastqStar {
    ...
    script:
        ...
        mv Aligned.sortedByCoord.out.bam ${sample_id}_STAR.bam
        """
}
...
process flagDuplicates {
    ...
    output:
        tuple val(sample_id),
            ...
            path("${sample_id}_dup_marked.bam.bai"),
            emit: flg_bams
    ...
    script:
        ...
        mv ${sample_id}_dup_marked.bai ${sample_id}_dup_marked.bam.bai
        """
}
...
workflow {
    ...
    flagDuplicates(alignFastqStar.out.aln_bams)
